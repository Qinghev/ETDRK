...
def run_etdrk_solver(etdrk_solver, dt_values, u0, interval, nt_base, reference_u):
    errors = []
    for dt in dt_values:
        nt = int(nt_base * dt_values[0] / dt)
        x, tt, uu = etdrk_solver(u0, interval, dt, nt, linear, nonlinear, show_plot=False)
...
_, _, uu_ref = burgers1d_etdrk4(deepcopy(u0), interval, dt_values[-1]/4, int(T / dt_values[-1]*4), linear, nonlinear, show_plot=False)
reference_u = uu_ref[-1]


rate2 = plot_convergence(
    dt_values,
    run_etdrk_solver(burgers1d_etdrk2, dt_values, u0, interval, nt_base, reference_u),
    "ETDRK2"
)

rate3 = plot_convergence(
    dt_values,
    run_etdrk_solver(burgers1d_etdrk3, dt_values, u0, interval, nt_base, reference_u),
    "ETDRK3"
)

rate4 = plot_convergence(
    dt_values,
    run_etdrk_solver(burgers1d_etdrk4, dt_values, u0, interval, nt_base, reference_u),
    "ETDRK4"
)
